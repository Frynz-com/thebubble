  ...
  return (
    <main className="min-h-svh overflow-hidden bg-surface" style={bubbleThemeStyle(config)}>
      ...
      <div className="phone-shell relative z-10 flex min-h-svh flex-col items-center justify-center px-5 pb-12 pt-24 text-center">
        ...
        <h1 className="text-[32px] font-bold leading-10 tracking-normal text-on-surface">Du bist dabei!</h1>
        <p className="mt-3 max-w-xs text-lg leading-7 text-on-surface-variant">{config.challengeDescription}</p>
        ...
        {config.features.rewards && config.rewardTitle ? (
          <Link href={bubblePath(bubbleSlug, "/benefits")} className="mt-10 flex w-full items-center gap-4 rounded-[1.5rem] border border-outline-variant/20 bg-white p-4 text-left shadow-ambient">
            ...
            <div>
              <p className="text-sm font-bold text-on-surface">{config.rewardTitle}</p>
              <p className="text-xs font-semibold text-on-surface-variant">{config.rewardTerms || config.rewardCode}</p>
            </div>
          </Link>
        ) : null}
      </div>
